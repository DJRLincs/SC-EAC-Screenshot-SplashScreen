...
class GameLauncher {
    ...
    start(opts) {
        return __awaiter(this, void 0, void 0, function* () {
            const { libraryFolder, gameName, channelId, nickname, token, authToken, hostname, port, installDir, executable, launchOptions, servicesEndpoint, network, } = opts;
            ...
            const rootInstallDirectory = path_1.default.resolve(completeLibraryFolderPath);
            ...
            const gameFilesManager = new game_files_manager_1.default(completeLibraryFolderPath, channelPath);
            ...
            const launcherData = {
                launcher_version: (0, global_config_1.getInstance)().launcherVersion,
            };
            const runOptions = {
                timeout: 0,
                maxBuffer: 200 * 1024,
            ...
            try {
                ...
                this.mainWindow.webContents.send(app_shared_1.ipcEvents.launcher.LAUNCHER_LAUNCH_SUCCESSFUL, Object.assign(opts, { runFile, isGameRunning: this.isGameRunning }));
                (0, analytics_1.sendAnalytics)({ name: app_shared_1.AnalyticsEventName.APP_LAUNCH_START }, this.mainWindow).then(() => electron_log_1.default.info('[Analytics] event sent : application launch game start'));
                ...
                this.mainWindow.webContents.send(app_shared_1.ipcEvents.launcher.LAUNCHER_LAUNCH_STOPPED, Object.assign(opts, { statistics: launchStatistics }));
                (0, analytics_1.sendAnalytics)({ name: app_shared_1.AnalyticsEventName.APP_LAUNCH_STOP }, this.mainWindow).then(() => electron_log_1.default.info('[Analytics] event sent : application launch game stop'));
            }
            catch (error) {
                if (error instanceof Error) {
                    ...
                    if (this.mainWindow.webContents) {
                        this.mainWindow.webContents.send(app_shared_1.ipcEvents.launcher.LAUNCHER_LAUNCH_FAILED, {
                ...
                (0, analytics_1.sendAnalytics)({ name: app_shared_1.AnalyticsEventName.APP_LAUNCH_ERROR }, this.mainWindow).then(() => electron_log_1.default.info('[Analytics] event sent : application launch game error'));
            }
            finally {
                // Notify the client of the current state at start up
                const gameFilesState = yield gameFilesManager.getGameFilesState();
                this.mainWindow.webContents.send(app_shared_1.ipcEvents.gameFiles.GAME_FILES_CHANGED, gameFilesState);
                this.isGameRunning = false;
