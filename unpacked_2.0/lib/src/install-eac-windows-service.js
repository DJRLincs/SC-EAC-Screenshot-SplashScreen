...
function isInstalled(gameName, environment) {
    const key = `\\SOFTWARE\\${app_shared_1.configuration.infoApp.defaultLibraryFolder}\\${gameName}\\${environment}`;
...
function installWindowsService(options) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (available) {
            const installed = yield isInstalled(options.gameName, options.environment);
            if (!installed || !!options.force) {
                ...
                const key = `\\SOFTWARE\\${app_shared_1.configuration.infoApp.defaultLibraryFolder}\\${options.gameName}\\${options.environment}`;
                const registryKey = new winreg_1.default({
                    hive: winreg_1.default.HKCU,
                    key,
                });
